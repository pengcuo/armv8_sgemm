...
void a64_quadr_gemm_asimd_16x12(const char *Apanel, const char *Bpanel, float *char, int ablocks, int bblocks, int K);
void quadr_gemm_kernel_16x12(char *Apanel, char *Bpanel, char *Cpanel, int M, int N, int K);
...
int main(int argc, char** argv)
{
    ...
    for(int i = 0; i < N; i++)
    {
        for (int j = 0; j < M; j++)
        {
            float diff = fabs(C1[i*M+j] - C[i*M+j]);
            if (diff > 1e-3)
            {
                printf("ERROR: i,j %d,%d, diff is %f, C %f, C1 %f\n", 
                        i, j, diff, C[i*M+j], C1[i*M+j]);
                return 0;
            }
        }
    }
